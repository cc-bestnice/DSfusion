...
def read_and_prepare_data(seq_len, pred_len, input_slice, static_channels):
    ...
    for file in files:
        ...
        single_train_x_static = torch.zeros(data_train.shape[0], static_channels, requires_grad=False)
        ...
        if len(features_test) >= seq_len + pred_len:
            ...
            single_test_x_static = torch.zeros(data_test.shape[0], static_channels, requires_grad=False)
...
if  data_type == 'diabetes':
    ...
    data_train_x_hist,data_train_x_static,data_train_y,data_test_x_hist,data_test_x_static,data_test_y = read_and_prepare_data(seq_len, pred_len, input_slice, static_channels)
...
for epoch in range(num_epochs):
    ...
    if pred_mse < best_mse:
        ...
        # 保存模型权重
        torch.save(model.state_dict(), 'model_file/TSMixer_9_8_0-1.pth')
    else:
        trigger_times += 1
...
model.load_state_dict(torch.load('model_file/TSMixer_9_8_0-1.pth'))
model.eval()
y_pred = model.forward(
    x_hist=data_test_x_hist.float().cuda(),
    x_static=data_test_x_static.float().cuda()
