...
class FeatureMixing(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        #根据 normalize_before 参数的值，选择在混合前或混合后应用归一化层。如果 normalize_before 为 True，
        # 则在输入端应用归一化，否则在输出端应用。如果不应用归一化，则使用 nn.Identity() 作为占位符。
        self.norm_before = (
        ...
        # self.projection 用于在需要时调整输入和输出维度的不一致情况。
        # 如果输入和输出通道数不同，则使用一个线性层进行维度调整，否则使用 nn.Identity()。
        self.projection = (
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        #接收一个形状为 (N, C, L) 的三维张量 x，其中 N 是批次大小，C 是通道数，L 是序列长度。
...
class ConditionalFeatureMixing(nn.Module):
    ...
    def forward(
        ...
        v = v.unsqueeze(1).repeat(
            1, x.shape[1], 1
        )  # 将静态特征扩展并重复以匹配动态特征的时间步长，使其在整个序列长度上保持一致。
        return (
            self.fm(
                torch.cat([x, v], dim=-1)
            ),  #  将扩展的静态特征和动态特征沿最后一个维度（特征维度）合并,对合并后的特征应用 FeatureMixing 模块
